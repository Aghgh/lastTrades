async function main() {
    ...
    function rateBet(bet) {
        ...
        if (bet.answerId) {rating -=0.2;} // personal dislike for multi-choice markets.
    ...
    async function loadAndOutputCorrespondingMarket(bet) {
        ...
        output("marketName: ", market.question + " ("+answer+") "+ market.uniqueBettorCount)
        ...
        outputLink(market.url)
        outputBold(bet.contractId);
        output(" ");
    }
    ...
    console.info("youngest Bet:", new Date(bets[0].createdTime).toLocaleTimeString())
    console.info("oldest   Bet:", new Date(bets[bets.length-1].createdTime).toLocaleTimeString())
    ...
    output("filtered out "+(betCount-bets.length)+" nothing bets")
    ...
    output("bets merged "+(newCount - bets.length));
    ...
    output("significant bets count:", bets.length )
    if (bets.length == 0) {
    ...
    for (let bet of bets) {
        output("amount: ", Math.round(bet._absAmount) + (bet._isSold ? "(sell)":""), 
            ...
            "    user: ", bet.userName || bet.userUsername || bet.userId)
        ...
        await loadAndOutputCorrespondingMarket(bet);
    }
...
setTimeout(()=>{
    ...
    console.log(globalThis.document?.getElementById("btn"))
})
//main();